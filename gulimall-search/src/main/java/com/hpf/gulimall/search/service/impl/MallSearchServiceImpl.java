...
import com.hpf.gulimall.search.vo.AttrResponseVo;
import com.hpf.gulimall.search.vo.BrandVO;
import com.hpf.gulimall.search.vo.SearchParam;
import com.hpf.gulimall.search.vo.SearchResult;
...
//        result.setPageNum();
        ...
        if (paramVo.getAttrs() != null && paramVo.getAttrs().size() > 0) {
            List<SearchResult.NavVo> collect = paramVo.getAttrs().stream().map(attr -> {
                ...
                if (r.getCode() == 0) {
                    AttrResponseVo attrs = r.getKey("attr", new TypeReference<AttrResponseVo>() {
                    });
        ...
        if (paramVo.getBrandId() != null && paramVo.getBrandId().size() > 0) {
            ...
            if (r.getCode() == 0) {
                List<BrandVO> brands = r.getKey("brands", new TypeReference<List<BrandVO>>() {
                });
